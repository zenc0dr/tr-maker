...
class Group extends FilterWidgetBase
{
    ...
    {
        $this->addJs('js/groupfilter.js', 'core');
    }
    ...
    public function onGetGroupOptions()
    {
        ...
        $activeKeys = (array) ($scope->scopeValue !== null ? $scope->value : $scope->default);
    ...
    protected function getOptionsFromArray($searchQuery = null)
    {
        ...
        if (is_scalar($options)) {
            ...
            if (!empty($scope->dependsOn)) {
                $options = $model->$methodName($this->getParentFilter()->getScopes());
            }
            else {
